...
with open(rl_agent_log_path, 'rb') as file:
    ...
    critic_loss_log, actor_loss_log, rewards_log, episode_lengths_log = data
...
mean_energy_create_plot(energy_by_episode[:], 'Cluster Total Energy Over Episode', 'Step', 'Energy', 'energy_by_e')
mean_energy_create_plot(active_by_episode[:], 'Cluster Total Number of Active nodes', 'Step', 'Number of Active Nodes', 'active_by_e')
mean_energy_create_plot(unique_by_episode[:], '', 'Step', 'Number of Unique Transmissions', 'unique_by_e')

#create_plot(throughput, 'Cluster Total Throughput', 'Step', 'Throughput (Packets)', 'throughput')
#create_plot(energy, 'Cluster Total Energy', 'Step', 'Energy', 'energy')
create_plot(episode_lengths_log, 'Episode Lengths', 'Episode', 'Length (Steps)', 'e_length')

#critic_loss_log = [l.detach().cpu().numpy() for l in critic_loss_log]
#actor_loss_log = [l.detach().cpu().numpy() for l in actor_loss_log]

create_plot(critic_loss_log, 'Critic Loss', 'Training Step', 'Loss', 'critic_loss')
create_plot(actor_loss_log, 'Actor Loss', 'Training Step', 'Loss', 'actor_loss')
